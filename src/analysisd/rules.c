...
static char *loadmemory(char *at, const char *str, char** error_str);
...
int Rules_OP_ReadRules(const char *rulefile, RuleNode **r_node, ListNode **l_node)
{
    ...
    if (OS_ReadXML(rulepath, &xml) < 0) {
        str_error_append(&error_str, XML_ERROR, rulepath, xml.err, xml.err_line);
        goto cleanup;
    }
    ...
    if (OS_ApplyVariables(&xml) != 0) {
        str_error_append(&error_str, XML_ERROR_VAR, rulepath, xml.err);
        goto cleanup;
    }
    ...
    if (!node) {
        str_error_append(&error_str, CONFIG_ERROR, rulepath);
        goto cleanup;
    }
    ...
    while (node[i]) {
        if (node[i]->element) {
            if (strcasecmp(node[i]->element, xml_group) != 0) {
                str_error_append(&error_str, "rules_op: Invalid root element \"%s\"."
            ...
            str_error_append(&error_str, XML_READ_ERROR);
            goto cleanup;
        }
        i++;
    }
    ...
    while (node[i]) {
        ...
        if (rule == NULL) {
            str_error_append(&error_str, "Group '%s' without any rule.",
                   node[i]->element);
            goto cleanup;
        }
        ...
        while (rule[j]) {
            ...
            if (strcasecmp(rule[j]->element, xml_rule) != 0) {
                str_error_append(&error_str, "Invalid configuration. '%s' is not "
            ...
            if ((!rule[j]->attributes) || (!rule[j]->values)) {
                str_error_append(&error_str, "Invalid rule '%d'. You must specify"
            ...
            /* Attribute block */
            {
                ...
                if (getattributes(rule[j]->attributes, rule[j]->values,
                                  ...
                                  &ignore_time, &overwrite, &error_str) < 0) {
                    str_error_append(&error_str, "Invalid attribute for rule.");
                    goto cleanup;
                }

                if ((id == -1) || (level == -1)) {
                    str_error_append(&error_str, "No rule id or level specified for "
                           "rule '%d'.", j);
                    goto cleanup;
                }

                if (overwrite != 1 && doesRuleExist(id, os_analysisd_rulelist)) {
                    str_error_append(&error_str, "Duplicate rule ID:%d", id);
                    goto cleanup;
                }
            ...
            /* Rule elements block */
            {
                ...
                if (rule_opt == NULL) {
                    str_error_append(&error_str, "Rule '%d' without any option. "
                ...
                while (rule_opt[k]) {
                    if ((!rule_opt[k]->element) || (!rule_opt[k]->content)) {
                        ...
                        if (config_ruleinfo->decoded_as == 0) {
                            str_error_append(&error_str, "Invalid decoder name: '%s'.",
                                   rule_opt[k]->content);
                            goto cleanup;
                        }
                        ...
                        if (!config_ruleinfo->day_time) {
                            str_error_append(&error_str, INVALID_CONFIG,
                                   rule_opt[k]->element,
                                   rule_opt[k]->content);
                            goto cleanup;
                        }
                        ...
                        if (!OS_IsValidIP(rule_opt[k]->content,
                                          config_ruleinfo->srcip[ip_s])) {
                            str_error_append(&error_str, INVALID_IP, rule_opt[k]->content);
                            goto cleanup;
                        }
                        ...
                        location = loadmemory(location, rule_opt[k]->content, &error_str);
                            ...
                            if (strcasecmp(rule_opt[k]->attributes[0], xml_name) == 0) {
                                ...
                                if (strcasecmp(rule_opt[k]->values[0], xml_srcuser) &&
                                    ...
                                    config_ruleinfo->fields[ifield]->name = loadmemory(config_ruleinfo->fields[ifield]->name,
                                                                                        rule_opt[k]->values[0], &error_str);
                                else {
                                    str_error_append(&error_str, "Field '%s' is static.", rule_opt[k]->values[0]);
                                    goto cleanup;
                                }

                            } else {
                                str_error_append(&error_str, "Bad attribute '%s' for field.", rule_opt[k]->attributes[0]);
                                goto cleanup;
                            }
                        } else {
                            str_error_append(&error_str, "No such attribute '%s' for field.", xml_name);
                            goto cleanup;
                        }
                        ...
                        if (!OSRegex_Compile(rule_opt[k]->content, config_ruleinfo->fields[ifield]->regex, 0)) {
                            str_error_append(&error_str, REGEX_COMPILE, rule_opt[k]->content, config_ruleinfo->fields[ifield]->regex->error);
                            goto cleanup;
                        }
                        ...
                        if (rule_opt[k]->attributes && rule_opt[k]->values && rule_opt[k]->content) {
                                ...
                                if (strcasecmp(rule_opt[k]->attributes[list_att_num], xml_list_lookup) == 0) {
                                    if (strcasecmp(rule_opt[k]->values[list_att_num], xml_match_key) == 0) {
                                        ...
                                        str_error_append(&error_str, INVALID_CONFIG,
                                               rule_opt[k]->element,
                                               rule_opt[k]->content);
                                        str_error_append(&error_str, "List match lookup=\"%s\" is not valid.",
                                    ...
                                    if (!OSMatch_Compile(rule_opt[k]->values[list_att_num], matcher, 0)) {
                                        str_error_append(&error_str, INVALID_CONFIG,
                                               rule_opt[k]->element,
                                               rule_opt[k]->content);
                                        str_error_append(&error_str, REGEX_COMPILE,
                                               rule_opt[k]->values[list_att_num],
                                               matcher->error);
                                        goto cleanup;
                                    }
                                } else {
                                    str_error_append(&error_str, "List field=\"%s\" is not valid",
                                           rule_opt[k]->values[list_att_num]);
                                    str_error_append(&error_str, INVALID_CONFIG,
                            ...
                            if (rule_type == 0) {
                                str_error_append(&error_str, "List requires the field=\"\" attribute");
                                str_error_append(&error_str, INVALID_CONFIG,
                                       rule_opt[k]->element, rule_opt[k]->content);
                                goto cleanup;
                            }
                            ...
                            if (config_ruleinfo->lists == NULL) {
                                str_error_append(&error_str, "List error: Could not load %s", rule_opt[k]->content);
                                goto cleanup;
                            }
                        } else {
                            str_error_append(&error_str, "List must have a correctly formatted field attribute");
                            str_error_append(&error_str, INVALID_CONFIG,
                                   rule_opt[k]->element,
                                   rule_opt[k]->content);
                            goto cleanup;
                        ...
                        if (!compiled_rules_name[it_id]) {
                            str_error_append(&error_str, "Compiled rule not found: '%s'",
                                   rule_opt[k]->content);
                            str_error_append(&error_str, INVALID_CONFIG,
                                   rule_opt[k]->element, rule_opt[k]->content);
                            goto cleanup;
                        ...
                        if (strcmp("alert_by_email",
                            ...
                            str_error_append(&error_str, XML_VALUEERR, xml_options,
                                   rule_opt[k]->content);

                            str_error_append(&error_str, "Invalid option '%s' for "
                ...
                if (!config_ruleinfo->comment) {
                    str_error_append(&error_str, "No such description at rule '%d'.", config_ruleinfo->sigid);
                    goto cleanup;
                }
                ...
                if ((config_ruleinfo->context_opts || config_ruleinfo->same_field ||
                    ...
                    str_error_append(&error_str, "Invalid use of frequency/context options. "
                ...
                if (regex) {
                    ...
                    if (!OSRegex_Compile(regex, config_ruleinfo->regex, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, regex,
                ...
                if (match) {
                    ...
                    if (!OSMatch_Compile(match, config_ruleinfo->match, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, match,
                ...
                if (id) {
                    ...
                    if (!OSMatch_Compile(id, config_ruleinfo->id, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, id,
                ...
                if (srcport) {
                    ...
                    if (!OSMatch_Compile(srcport, config_ruleinfo->srcport, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, srcport,
                               config_ruleinfo->id->error);
                        goto cleanup;
                    }
                    free(srcport);
                ...
                if (dstport) {
                    ...
                    if (!OSMatch_Compile(dstport, config_ruleinfo->dstport, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, dstport,
                               config_ruleinfo->id->error);
                        goto cleanup;
                    }
                    free(dstport);
                ...
                if (status) {
                    ...
                    if (!OSMatch_Compile(status, config_ruleinfo->status, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, status,
                ...
                if (hostname) {
                    ...
                    if (!OSMatch_Compile(hostname, config_ruleinfo->hostname, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, hostname,
                ...
                if (data) {
                    ...
                    if (!OSMatch_Compile(data,
                        ...
                        str_error_append(&error_str, REGEX_COMPILE, data,
                ...
                if (extra_data) {
                    ...
                    if (!OSMatch_Compile(extra_data,
                        ...
                        str_error_append(&error_str, REGEX_COMPILE, extra_data,
                ...
                if (program_name) {
                    ...
                    if (!OSMatch_Compile(program_name,
                        ...
                        str_error_append(&error_str, REGEX_COMPILE, program_name,
                ...
                if (user) {
                    ...
                    if (!OSMatch_Compile(user, config_ruleinfo->user, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, user,
                ...
                if(srcgeoip) {
                    ...
                    if(!OSMatch_Compile(srcgeoip, config_ruleinfo->srcgeoip, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, srcgeoip,
                ...
                if(dstgeoip) {
                    ...
                    if(!OSMatch_Compile(dstgeoip, config_ruleinfo->dstgeoip, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, dstgeoip,
                ...
                if (url) {
                    ...
                    if (!OSMatch_Compile(url, config_ruleinfo->url, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, url,
                ...
                if (location) {
                    ...
                    if (!OSMatch_Compile(location, config_ruleinfo->location, 0)) {
                        str_error_append(&error_str, REGEX_COMPILE, location, config_ruleinfo->location->error);
                        goto cleanup;
                    }
                    free(location);
                    location = NULL;
                ...
                if (if_matched_group) {
                    ...
                    if (!OSMatch_Compile(if_matched_group,
                                         config_ruleinfo->if_matched_group,
                                         0)) {
                        str_error_append(&error_str, REGEX_COMPILE, if_matched_group,
                ...
                if (if_matched_regex) {
                    ...
                    if (!OSRegex_Compile(if_matched_regex,
                        ...
                        str_error_append(&error_str, REGEX_COMPILE, if_matched_regex,
                ...
                if(protocol){
                    ...
                    if(!OSMatch_Compile(protocol, config_ruleinfo->protocol, 0)){
                        str_error_append(&error_str, REGEX_COMPILE, protocol, config_ruleinfo->protocol->error);
                        goto cleanup;
                    }
                    free(protocol);
                    protocol = NULL;
                ...
                if(system_name){
                    ...
                    if(!OSMatch_Compile(system_name, config_ruleinfo->system_name, 0)){
                        str_error_append(&error_str, REGEX_COMPILE, system_name, config_ruleinfo->system_name->error);
                        goto cleanup;
                    }
                    free(system_name);
                    system_name = NULL;
            ...
            if (config_ruleinfo->sigid < 10) {
                ...
                if (!OS_AddRuleInfo(*r_node, config_ruleinfo,
                                    config_ruleinfo->sigid)) {
                    str_error_append(&error_str, "Overwrite rule '%d' not found.",
                           config_ruleinfo->sigid);
                    goto cleanup;
                }
            } else {
...
static char *loadmemory(char *at, const char *str, char** error_str)
{
    if (at == NULL) {
        ...
        if ((strsize = strlen(str)) < OS_SIZE_2048) {
            ...
            str_error_append(error_str, SIZE_ERROR, str);
            return (NULL);
        }
    } else {
...
int get_info_attributes(char **attributes, char **values, char** error_str)
{
    ...
    while (attributes[k]) {
        if (strcasecmp(attributes[k], xml_type) == 0) {
            if (!values[k]) {
                str_error_append(error_str, "rules_op: Element info attribute \"%s\" does not have a value",
                       attributes[k]);
                return (-1);
                ...
                str_error_append(error_str, "rules_op: Element info attribute \"%s\" has invalid value \"%s\"",
                       attributes[k], values[k]);
                return (-1);
            }
        } else {
            str_error_append(error_str, "rules_op: Element info has invalid attribute \"%s\"",
                   attributes[k]);
            return (-1);
        }
    }
                  ...
                  char** error_str)
{
    ...
    while (attributes[k]) {
        if (!values[k]) {
            str_error_append(error_str, "rules_op: Attribute \"%s\" without value."
                   , attributes[k]);
            return (-1);
        }
        /* Get rule id */
        ...
        /* Get frequency */
            ...
            if (OS_StrIsNum(values[k])) {
                ...
                *frequency = *frequency - 2;
                ...
                str_error_append(error_str,"rules_op: Invalid frequency: %s. "
                       "Must be integer" ,
                       values[k]);
                return (-1);
            }
        ...
        /* Rule accuracy */
            ...
            if (OS_StrIsNum(values[k])) {
                ...
                str_error_append(error_str, "rules_op: Invalid accuracy: %s. "
                       "Must be integer" ,
                       values[k]);
                return (-1);
            }
        ...
        /* Rule ignore_time */
            ...
            if (OS_StrIsNum(values[k])) {
                ...
                str_error_append(error_str, "rules_op: Invalid ignore_time: %s. "
                       "Must be integer (max 6 digits)" ,
                       values[k]);
                return (-1);
            }
            ...
            *noalert = 1;
            ...
            if (strcmp(values[k], "yes") == 0) {
                ...
                *overwrite = 0;
                ...
                str_error_append(error_str,"rules_op: Invalid overwrite: %s. "
                       "Can only by 'yes' or 'no'.", values[k]);
                return (-1);
            }
        } else {
            str_error_append(error_str, "rules_op: Invalid attribute \"%s\". "
